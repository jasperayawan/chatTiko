...
const Conversation = ({ conversation }) => {
  ...
  return (
    <div onClick={() => dispatch(setSelectedConversation(conversation))} className={`p-2 cursor-pointer rounded-2xl hover:bg-[#2e333d] ${isSelected && 'bg-[#2e333d]'}`}>
      <div className="flex flex-row justify-between items-center">
        <div className="flex flex-row  gap-x-4 justify-center items-center">
            ...
            <div className="flex flex-col">
                <h3 className='text-base text-[#ffffff]'>{conversation.fullName}</h3>
                <p className='text-sm text-slate-500'>{mssg}</p>
            </div>
        </div> 
